...
# If the user only provides an interface e.g
# network-config-expanded.sh ens34
[[ -z $verbose ]] && [[ -z $interface ]] && { 
	if [ $# -eq 1 ];then
			interface="$1"
	fi
}
...
all_interfaces=$( nmcli device status | tail -n +2 | awk '{print $1}' )
[[ -z $interface ]] || all_interfaces=($interface) 
